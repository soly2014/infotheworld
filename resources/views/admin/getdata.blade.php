...
@section('adminContent')
    ...
    @foreach($countries as $country)
        ...
        <?php
        $cCheck = \App\visit::where('place', $country->name)->first();
        ?>
        @if(count($cCheck) == 0)
            <script>
                try {
                    var url = "https://maps.googleapis.com/maps/api/geocode/json?address={{$country->name}}&key=AIzaSyCLkUMzd2rh5ThGMOzGHUuDhxFt7vinMtY&language=en";
